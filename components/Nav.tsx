...
const Nav: React.FC = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 bg-black/80 backdrop-blur-sm transition-transform duration-300 ${visible ? "translate-y-0" : "-translate-y-full"}`}>
      <div className="mx-auto max-w-[2000px] px-4 md:px-6 lg:px-9 pt-4 pb-3 md:pt-8 md:pb-6">
        <nav className="flex flex-wrap items-center justify-between gap-x-6 gap-y-3 md:grid md:grid-cols-3 md:items-start md:gap-8">
          ...
          <div className="flex flex-col md:items-center">
            <span className="hidden md:block text-sm font-bold text-white uppercase tracking-wider">Quick Links</span>
            <div className="flex gap-2 md:mt-1 text-sm">
              {LINKS.map((link, i) => (
        ...
        <div className="mt-3 md:mt-4 border-b border-white/10" />
      </div>
    </header>
  );
};
